...
try {
  ...
  @{ sha = $resp.content.sha } | ConvertTo-Json | Set-Content -Path $MetaPath -Encoding UTF8
  Write-Host "MEME_CAPSULE_KNOWLEDGE.md updated successfully! (new sha: $($resp.content.sha.Substring(0,7)))"
} catch {
  $statusCode = 0
  $errorMsg = ""
  ...
  if ($statusCode -eq 409) {
    ...
    Write-Host "   (Alternatively, generate a Classic PAT with the 'repo' scope checkbox checked)."
